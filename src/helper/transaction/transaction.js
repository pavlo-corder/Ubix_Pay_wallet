...
class Transaction {
  ...
  getHash() {
    ...
    return Crypto.createHash(
      transactionPayloadProto.encode(this._data.payload).finish()
    );
  }
  ...
  getTxSignerAddress(needBuffer = false) {
    ...
    try {
      const pubKey = Crypto.recoverPubKey(
        this.getHash(),
        this.getTxSignature()
      );
  ...
  verify() {
    ...
    assert(
      this.claimProofs.length === inputs.length || this.getTxSignature(),
      "Errors in clamProofs"
    );
  }
  ...
  isContractCreation() {
    ...
    return outCoins[0]
      .getReceiverAddr()
      .equals(Crypto.getAddrContractCreation());
  }
